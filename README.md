...
1. **来店登録モーダル**: 顧客詳細画面からクイック入力
   - 日時、金額、指名タイプ、メモの入力
   - `/api/customers/:id/visits` POSTで登録
   
2. **メッセージ送信画面**: テンプレート選択・編集・送信
   - テンプレート一覧から選択
   - 変数置換プレビュー（{castName}, {customerName}等）
   - ガードレール確認（時間帯、送信頻度）
   - `/api/messages/send` POSTで送信
   
3. **顧客編集モーダル**: 呼び名、メモ、タグの編集
   - `/api/customers/:id` PATCHで更新

### 高優先度

1. **マネージャー管理画面**: PC向け管理画面
   - ダッシュボード（統計・グラフ）
   - キャスト管理（追加・無効化）
   - テンプレート管理（CRUD）
   - ToDo生成ルール設定
...
**Status**: MVP 90% Complete - Backend + Cast Frontend Operational
...
- ✅ D1認証問題解決（Raw SQL使用）
- ✅ キャスト向けフロントエンドUI実装完了
- ✅ ログイン・ホーム・顧客一覧・詳細・登録コード画面
- 🚧 来店登録・メッセージ送信モーダル（未実装）
- ❌ マネージャー管理画面（未実装）
